...
contract DeployScript is Script {
    function run() public {
        ...
        string memory path = "./api/.env"; // The path is relative to the root of the project folder.
        string memory data = "PORT="; // Append vm.envUint("PORT");
        data = string(abi.encodePacked(data, Strings.toString(vm.envUint("PORT"))));
        ...
        data2 = string(abi.encodePacked(data2, Strings.toString(vm.envUint("PORT"))));
        data2 = string(abi.encodePacked(data2, "\nREACT_APP_API_URL=")); // Append vm.envString("API_URL");
